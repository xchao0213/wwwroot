...
exec="select * from fengfa where id="&id
rs.open exec,conn,1,1
if rs.eof then
response.Write "<div style=""padding:10px"">无此园区！</div>"
response.End()
else
title=rs("title")
qx=rs("qx")
...
exec="select * from fengfa_fl where id="&ssfl
...
elseif html=1 then
newsdaohang="<a href="""&dir&"Fengfa/Fengfalist.asp?id="&fl("id")&""">"&fl("title")&"</a>"
end if
fl.close
set lf=nothing%>
...
<div class="bar"><b><%=zych_pd_class(wurl)%></b><span>当前位置：<a href="/">首页</a><a href="<%=dir%>Fengfa/"><%=zych_class(wurl)%></a><%=newsdaohang%></span></div>
<div class="w_650 fl">
<div class="main_content">
<h2 class="Showtitle"><%=title%></h2>
<div class="ShowDetails">作者：<%=zz%> 来源：<%=ly%> 发表时间：<%=data%>&nbsp;查看：<script src="/Include/Click.asp?click=Fengfa&id=<%=id%>"></script>次</div>
...
<%
if MaxPages >0 then
url=""&dir&"Fengfa/ShowFengfa.asp?id="&id
...
rstmp.open "select top 1 id, title from fengfa where id>"&id&" order by id asc",conn,1,1
if not rstmp.eof then
if html=0 then
nexttitle= "下一篇：<a href="""&dir&"Culture/"&ShowNews&Separated&rstmp(0)&"."&HTMLName&""">"&stvalue(rstmp(1),60)&"</a>"
elseif html=1 then
nexttitle="下一篇：<a  href="""&dir&"Fengfa/ShowFengfa.asp?id="&rstmp(0)&""" title="""&stvalue(rstmp(1),60)&""">"&stvalue(rstmp(1),60)&"</a>"
end if
else
nexttitle = "下一篇：<a>已经没有了！</a>"
end if
rstmp.close
rstmp.open "select top 1 id, title from fengfa where id<"&id&" order by id desc"
if not rstmp.eof then
if html=0 then
prevtitle="上一篇：<a href="""&dir&"Culture/"&ShowNews&Separated&rstmp(0)&"."&HTMLName&""" >"&stvalue(rstmp(1),60)&"</a>"
elseif html=1 then
prevtitle="上一篇：<a href="""&dir&"Fengfa/ShowFengfa.asp?id="&rstmp(0)& """ title="""&stvalue(rstmp(1),60)&""">"&stvalue(rstmp(1),60)&"</a>"
end if
else
