...
<td width="19%" height="25" align="center" class="td">标 题</td>
<td width="22%" align="center" class="td">缩略图</td>
<td width="14%" align="center" class="td">排序</td>
<td width="21%" align="center" class="td">时间</td>
...
<td width="19%" height="25" align="center" class="td"><a href="admin_flash.asp?action=xuigai&id=<%=rs("id")%>" style="color:#003399"><%=rs("title")%></a></td>
<td width="22%" align="center" class="td"><a href="<%=rs("img")%>"><img src="<%=rs("img")%>" width="70" height="28" border="0" /></a></td>
<td width="14%" align="center" class="td"><input name="px_id" type="text" style="text-align:center; width:40px" value="<%=rs("px_id")%>"/></td>
<td width="21%" align="center" class="td"><%=rs("tim")%></td>
<td width="6%" align="center" class="td">
<input type="button" name="Submit3" value="修改" onclick="window.location.href='admin_flash.asp?action=xuigai&id=<%=rs("id")%>' "  class="btn"/></td>
...
<td colspan="7"><input type="submit" class="btn" onclick="form.action='?action=admin&act=del';" value="删除"/>
...
<form  name="add" method="post" action="admin_flash.asp?add=ok">
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC">
  <tr>
    <td height="30" background="images/bg_list.gif"><div  style="padding-left:10px; font-weight:bold; color:#FFFFFF">FLASH幻灯添加</div></td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td width="62%"><iframe src="up.asp?formname=add&inputname=img&uploadstyle=Ad" frameborder="0" scrolling="no" width="350" height="21"></iframe></td>
  </tr>
</table>
          </td>
        </tr>
...
<%end if'以下为修改
if request.querystring("action")="xuigai" then%>
...
<form  name="myform" method="post" action="?id=<%=rs("id")%>&xiugai=ok">
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC">
  <tr>
  ...
  <tr>
    <td bgcolor="#FFFFFF"><table width="100%" border="0" align="center" cellpadding="5" cellspacing="0" class="stable" >
      ...
      <tr bgcolor="#ffffff">
        ...
        <td colspan="2" class="td"><table width="90%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td width="30%"><input type="text" name="img" id="url" size="50" value="<%=rs("img")%>"></td>
...
conn.close
set rs=nothing
Response.Write "<script>alert('广告添加成功！点击继续添加！');window.location.href='admin_flash.asp?action=admin';</script>" 
end if
...
set rs=server.createobject("adodb.recordset")
sql="select * from flash order by px_id asc"
rs.open sql,conn,1,3
...
rs.close
set rs=nothing
Response.Write "<script>alert('更新排序操作成功!');window.location.href='admin_flash.asp?action=admin';</script>" 
end if
...
rs.delete
rs.update
Response.Write "<script>alert('删除成功！');window.location.href='admin_flash.asp?action=admin';</script>"
end if 
...
sql="delete from [flash] where id in ("&Request("id")&")"
conn.execute(sql) 
Response.Write "<script>alert('恭喜!删除成功!');window.location.href='admin_flash.asp?action=admin';</script>" 
end if
%>
...
rs.update 
rs.close 
response.write "<script>alert('幻灯更新成功！');window.location.href='admin_flash.asp?action=admin';</script>" 
end if
%> 