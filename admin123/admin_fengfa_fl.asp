...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC">
  <tr>
    <td height="30" background="images/bg_list.gif"><div  style="padding-left:10px; font-weight:bold; color:#FFFFFF">园区分类管理</div></td>
  </tr>
  <tr>
...
exec="select * from fengfa_fl order by px_id asc" 
...
<form action="admin_fengfa_fl.asp?xiugai=ok" method="post" name="add">
<tr>
...
<td width="29%" align="center" class="td"><input name="url" type="text" value="/Fengfa/Fengfalist.asp?id=<%=rs("id")%>" size="20"/> 
[<a href="/Fengfa/Fengfalist.asp?id=<%=rs("id")%>" target="_blank" style="color:#003399">访问</a>]</td>
...
<td width="7%" align="center" class="td"><input type="button" name="Submit" value="删除" onClick="javascript:if(confirm('确定删除？删除后不可恢复!')){window.location.href='admin_fengfa_fl.asp?act=del&id=<%=rs("id")%>';}else{history.go(0);}"  class="btn"/></td>
</tr></form>
<% rs.movenext 
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC">
<form action="admin_fengfa_fl.asp?add=ok" method="post" name="add">
  <tr>
...
conn.close
set rs=nothing
Response.Write "<script>alert('分类增加成功！');window.location.href='admin_fengfa_fl.asp';</script>" 
end if
%>
<% 
if Request.QueryString("xiugai")="ok" then
id=request("id") 
sql="select * from fengfa_fl where id="&id 
set rs=server.CreateObject("adodb.recordset")
rs.open sql,conn,1,3
IF not isNumeric(request("px_id")) then
...
rs.update 
rs.close 
response.Write("<script language=""javascript"">alert(""当前分类修改成功！"");window.location.href='admin_fengfa_fl.asp';</script>")
end if
%> 
<%
if request("act")="del" then
	id=request("id")
	if id="" then
	Response.Write "<script language='javascript'>alert('参数错误!');document.location.href('admin_fengfa_fl.asp');</script>"
	Response.End()
	end if
set rs=server.createobject("adodb.recordset")
rs.open "Select * from fengfa_fl where id="&Request("id"),conn,1,3
if rs.bof and rs.eof then
	Response.Write "<script language='javascript'>alert('数据库中没有该记录！');document.location.href('admin_fengfa_fl.asp');</script>"
	Response.End()
else
	rs.Delete
	rs.Update
	Response.Write "<script language='javascript'>alert('当前分类删除成功！');document.location.href('admin_fengfa_fl.asp');</script>"
end if
end if
%>
