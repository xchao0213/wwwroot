...
exec="select * from fengfa_fl where id="&rs("ssfl")&""
...
<div class="post_info"> <span class="posted_by">Author：<%=rs("zz")%>&nbsp; Source：<%=rs("ly")%>&nbsp; hot：&nbsp;<script src="/Include/Click.asp?click=Fengfa&id=<%=id%>"></script></span></div>
...
<%
 ...
 rstmp.open "select top 1 id, title from fengfa where id>"&request.QueryString("id")&" and ssfl="&daohang("id")&" order by id asc",conn,1,1
 ...
 rstmp.close
 rstmp.open "select top 1 id, title from fengfa where id<"&request.QueryString("id")&" and ssfl="&daohang("id")&" order by id desc"
 if not rstmp.eof then
