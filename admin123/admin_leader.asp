...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC">
  <tr>
    <td height="30" background="images/bg_list.gif"><div  style="padding-left:10px; font-weight:bold; color:#FFFFFF">领导关心项目管理</div></td>
  </tr>
  <tr>
    <td bgcolor="#FFFFFF">
...
if rs.eof then
response.write ("<div style=""padding:10px;"">暂无领导关心项目!</div>")
else
...
<tr>
  ...
  <td width="92" align="center" class="td"><input type="button" name="Submit3" value="修改" onclick="window.location.href='admin_leader.asp?action=xiugai&id=<%=rs("id")%>' "  class="btn"/></td>
  <td width="93" align="center" class="td"><input type="button" name="Submit" value="删除" onclick="javascript:if(confirm('确定删除？删除后不可恢复!')){window.location.href='admin_leader.asp?action=admin&id=<%=rs("id")%>&del=ok';}else{history.go(0);}"  class="btn"/></td>
</tr>
<%rs.movenext 
...
<form action="admin_leader.asp?add=ok" method="post" name="add">
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC">
 <tr>
    <td height="30" background="images/bg_list.gif"><div  style="padding-left:10px; font-weight:bold; color:#FFFFFF">修改领导关心</div></td>
  </tr>
  <tr>
    <td bgcolor="#FFFFFF"><table width="100%" border="0" align="center" cellpadding="5" cellspacing="0" >
      <tr  >
        <td width="13%" height="28" align="right" class="td"><font color="#FF0000">*</font>领导关心标题</td>
...
<tr >
     ...
     <tr >
        <td height="25" align="right" class="td"><font color="#FF0000">*</font>领导关心详情</td>
...
exec="select * from leader where id="& request.QueryString("id") 
set rs=server.createobject("adodb.recordset") 
rs.open exec,conn,1,1 
...
<form  name="add" method="post" action="admin_leader.asp?action=xiugai&id=<%=rs("id")%>&xiugai=ok"> 
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC">
 <tr>
    <td height="30" background="images/bg_list.gif"><div  style="padding-left:10px; font-weight:bold; color:#FFFFFF">修改领导关心</div></td>
  </tr>
...
rs.delete
rs.update
Response.Write "<script>alert('当前照片删除成功！');window.location.href='admin_leader.asp?action=admin';</script>"
end if 
%>
<%
...
sql="delete from leader where id in ("&Request("id")&")"
conn.execute(sql) 
Response.Write "<script>alert('恭喜!删除成功!');window.location.href='admin_leader.asp?action=admin';</script>" 
end if
'=====================================
...
set rs=server.createobject("adodb.recordset")
sql="select * from leader order by px_id asc"
...
rs.close
set rs=nothing
Response.Write "<script>alert('排序成功！');window.location.href='admin_leader.asp?action=admin';</script>"
end if
...
conn.close
set rs=nothing
Response.Write "<script>alert('项目增加成功！');window.location.href='admin_leader.asp?action=admin';</script>" 
end if
...
if name=""  then 
response.Write("<script language=javascript>alert('领导关心名称不能为空!');history.go(-1)</script>") 
response.end 
end if
if px_id=""  then 
response.Write("<script language=javascript>alert('排序ID不能为空!');history.go(-1)</script>") 
...
rs.update 
rs.close 
response.write "<script>alert('领导关心项目修改成功！');window.location.href='admin_leader.asp?action=admin';</script>" 
end if
%> 
