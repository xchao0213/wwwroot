...
<td height="25" align="center" class="td">新闻标题</td>
...
<td height="25" class="td"><a href="xiugai_news.asp?id=<%=rs("id")%>"><%=stvalue(DelHtml(rs("title")),40)%></a>  </td>
...
<td align="center" class="td"><input type="button" name="Submit3" value="修改" onclick="window.location.href='xiugai_news.asp?id=<%=rs("id")%>'"  class="btn"/></td>
<td align="center" class="td"><input type="button" name="Submit" value="删除" onclick="javascript:if(confirm('确定删除？删除后不可恢复!')){window.location.href='admin_news.asp?id=<%=rs("id")%>&amp;del=ok';}else{history.go(0);}"  class="btn"/></td>
</tr>
<%rs.movenext 
...
rs.delete
rs.update
Response.Write "<script>alert('当前新闻删除成功！');window.location.href='admin_news.asp?page="&page&"';</script>"
end if 
...
conn.execute(sql) 
Response.Write "<script>alert('恭喜!推荐成功!');window.location.href='admin_news.asp?page="&page&"';</script>" 
end if
...
conn.execute(sql) 
Response.Write "<script>alert('恭喜!取消推荐成功!');window.location.href='admin_news.asp?page="&page&"';</script>" 
end if
...
conn.execute(sql) 
Response.Write "<script>alert('恭喜!删除成功!');window.location.href='admin_news.asp?page="&page&"';</script>" 
end if
