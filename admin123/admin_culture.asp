...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC">
  <tr>
    <td height="30" background="images/bg_list.gif"><div  style="padding-left:10px; font-weight:bold; color:#FFFFFF">文化管理</div></td>
  </tr>
  <tr>
    <td bgcolor="#FFFFFF">
<div style="text-align:left; height:25px; padding:2px 10px 2px 10px;"> 
<%set rsk=server.CreateObject("adodb.recordset")
rsk.open "select * from culture_fl order by id asc",conn,1,1
if rsk.eof then
response.write""
else
...
response.Write("<option value=""?id="">全部文化</option>")
while not rsk.eof
...
if id<>"" then
exec="select * from culture where ssfl="& id &"  order by id desc" 
else
exec="select * from culture order by id desc"  
end if
...
<td height="25" class="td"><a href="xiugai_culture.asp?id=<%=rs("id")%>"><%=stvalue(DelHtml(rs("title")),40)%></a>  </td>
...
<td align="center" class="td"><input type="button" name="Submit3" value="修改" onclick="window.location.href='xiugai_culture.asp?id=<%=rs("id")%>'"  class="btn"/></td>
<td align="center" class="td"><input type="button" name="Submit" value="删除" onclick="javascript:if(confirm('确定删除？删除后不可恢复!')){window.location.href='admin_culture.asp?id=<%=rs("id")%>&amp;del=ok';}else{history.go(0);}"  class="btn"/></td>
</tr>
<%rs.movenext 
...
rs.delete
rs.update
Response.Write "<script>alert('当前文化删除成功！');window.location.href='admin_culture.asp?page="&page&"';</script>"
end if 
...
sql="update culture set tuijian=1 where id in ("&Request("id")&")" 
conn.execute(sql) 
Response.Write "<script>alert('恭喜!推荐成功!');window.location.href='admin_culture.asp?page="&page&"';</script>" 
end if
...
sql="update culture set tuijian=0 where id in ("&Request("id")&")" 
conn.execute(sql) 
Response.Write "<script>alert('恭喜!取消推荐成功!');window.location.href='admin_culture.asp?page="&page&"';</script>" 
end if
...
sql="delete from culture where id in ("&Request("id")&")"
conn.execute(sql) 
Response.Write "<script>alert('恭喜!删除成功!');window.location.href='admin_culture.asp?page="&page&"';</script>" 
end if
